...
// ---------------------------------------------------------------------------
// TOOLS — Dynamic Studio Library Queries
// ---------------------------------------------------------------------------
  ...
  async ({ category, include_stories, include_notes }) => {
    ...
    try {
      ...
      return {
        content: [{
          type: "text" as const,
          text: `Error querying Studio library: ${error instanceof Error ? error.message : String(error)}`,
        }],
        isError: true,
      };
    }
...
including community notes, linked prompts, and usage guidance.`,
  ...
  async ({ tool_name }) => {
    try {
      // Search for the tool by name (case-insensitive partial match)
      const tools = await studioFetch(
      ...
      if (!Array.isArray(tools) || tools.length === 0) {
        return {
          content: [{
            type: "text" as const,
            text: `No tools found matching "${tool_name}". Try searching the full library with search-studio-library.`,
          }],
        };
      }
      ...
      return {
        content: [{
          type: "text" as const,
          text: `Error fetching tool details: ${error instanceof Error ? error.message : String(error)}`,
        }],
        isError: true,
      };
    }
...
live Studio library.`,
  ...
  async ({ situation, builder_stage, primary_need }) => {
    ...
    // Match builder stage to guidance
    if (builder_stage) {
    ...
    // Match primary need to playbooks and tools
    if (primary_need) {
    ...
    // Also search the Studio library for relevant tools
    try {
      const tools = await studioFetch(
        `/tools?select=name,summary,tool_category,url`
      ) as Array<Record<string, unknown>>;
      ...
      // Fetch stories for inspiration
      const stories = await studioFetch(
        `/stories?select=title,story_text,attribution`
      ) as Array<Record<string, unknown>>;
...
const NETWORK_FEED_URL = "https://updates.relationaltechproject.org/feed.json";

const CONTRIBUTION_NUDGE_NETWORK = `\n\n---\n\n## The Network Grows When You Share\n\nIf you're building something for your neighborhood, the network gets richer when others can see your work. Tag your repo with \`relational-tech\` on GitHub and it will automatically appear in this feed. You can also contribute patterns, stories, and tools to the Studio commons.\n\nJust say **"I'd like to share what I'm building with the network"** and your AI tool can help.`;
...
server.tool(
  ...
  async ({ repo_name, tag, entry_type, limit }) => {
    ...
    try {
      const res = await fetch(NETWORK_FEED_URL);
      if (!res.ok) {
        return {
      ...
      if (entries.length === 0) {
        ...
        return {
          content: [{ type: "text" as const, text: `No updates found${filters ? ` matching ${filters}` : ""}. The network is quiet right now — but that just means there's room for your project to be the next update!` + CONTRIBUTION_NUDGE_NETWORK }],
        };
      }
...
1. First, understand this builder's context deeply. Ask about their
   ...
   Use the builder intake framework: identity & place, existing assets,
   current challenge, and readiness.
...
- Completable in under 2 hours each
...
- Could a different builder get this same plan? If yes, revise.
